...
const RSP_DIR_TO_SEARCH = __dirname + `/../../rsp/distribution/distribution.mytype/target/`
...
function findZip(basedir) {
	...
	fs.readdirSync(basedir).forEach(file => {
		if( file.endsWith("zip") && !file.includes("distribution-")) {
			result = file;
		}
	});
	console.log("Copying server distribution " + result);
	return result;
}
